...
test ('login' , async ({page}) => {
    ...
    for(let i=0; i<productCount; i++){
        ...
        if(productName.trim() == productToSearch){
            await page.screenshot({path : 'screenshotProduct.png'})
    ...
    await cartBtn.screenshot({path : 'element.png'});
    ...
    await page.screenshot({path : 'checkoutPage.png'});
    ...
    await page.locator("[class^='input txt']").nth(2).fill('677');
    await page.locator("[class^='input txt']").nth(3).fill('Surya');
    await page.locator("[class^='input txt']").nth(4).fill('rahulshettyacademy');
    await page.locator("button:has-text('Apply Coupon')").click();
    const coupon = await page.locator('[name="coupon"] + p').isVisible();
